...
<Info>
  **Help others by sharing your setup!** If you have a working configuration that isn't covered here, I'd love to include it. Simply [open a pull request](https://github.com/amruthpillai/Resume-Craft) with your example added to this page. Your contribution helps the community and makes self-hosting easier for everyone.
</Info>
...
This example uses [Traefik](https://traefik.io/) as a reverse proxy with automatic SSL certificate management via Let's Encrypt. Only the Resume Craft app is exposed through Traefik—Postgres and the printer remain on an internal network.
...
services:
  ...
  reactive_resume:
    ...
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.Resume-Craft.rule=Host(`resume.${DOMAIN}`)"
      - "traefik.http.routers.Resume-Craft.entrypoints=websecure"
      - "traefik.http.routers.Resume-Craft.tls.certresolver=letsencrypt"
      - "traefik.http.services.Resume-Craft.loadbalancer.server.port=3000"
    healthcheck:
...
services:
  ...
  reactive_resume:
    ...
    deploy:
      ...
      labels:
        - "traefik.enable=true"
        - "traefik.http.routers.app.rule=Host(`Resume-Craft.vercel.app`)"
...
To contribute, [open a pull request](https://github.com/amruthpillai/Resume-Craft) with your example added to this page. Include:

...
Resume Craft supports custom OAuth providers, allowing you to integrate with enterprise identity providers and self-hosted authentication solutions. This is particularly useful for organizations that want to:
...
<Info>
  Custom OAuth is designed for **self-hosted instances**. If you're using the hosted version at [Resume-Craft.vercel.app](https://Resume-Craft.vercel.app), you can use the built-in Google and GitHub sign-in options.
</Info>
...
<Tabs>
  <Tab title="Option A: OIDC Discovery (Recommended)">
    ...
    **Examples:**
    - Authentik: `https://auth.example.com/application/o/Resume-Craft/.well-known/openid-configuration`
...
Resume Craft automatically maps user profile data from the OAuth provider. The following fields are used:

| Resume Craft Field | OAuth Profile Fields (in order of preference) |
|-----------------------|-----------------------------------------------|
...
OAUTH_DISCOVERY_URL="https://auth.example.com/application/o/Resume-Craft/.well-known/openid-configuration"
```
  </Step>
</Steps>
...
identity_providers:
  oidc:
    clients:
      - client_id: Resume-Craft
        client_name: Resume Craft
...
OAUTH_CLIENT_ID="Resume-Craft"
OAUTH_CLIENT_SECRET="your-plain-secret"
OAUTH_DISCOVERY_URL="https://auth.example.com/.well-known/openid-configuration"
```

    <Warning>
      Use the **plain text** secret in Resume Craft's environment, not the hashed version used in Authelia's configuration.
    </Warning>
  </Step>
</Steps>
...
<Steps>
  <Step title="Create a client">
    ...
    3. Set **Client ID** (e.g., `Resume-Craft`)
...
OAUTH_CLIENT_ID="Resume-Craft-client-id"
OAUTH_CLIENT_SECRET="Resume-Craft-client-secret"
OAUTH_DISCOVERY_URL="https://auth.company.com/application/o/Resume-Craft/.well-known/openid-configuration"
...
<CardGroup cols={2}>
  <Card title="Use HTTPS" icon="lock">
    Always use HTTPS for both your Resume Craft instance and OAuth provider in production. OAuth tokens should never be transmitted over unencrypted connections.
  </Card>
  ...
  <Card title="Review scopes" icon="list-check">
    Only request the scopes you need. The default (`openid profile email`) is sufficient for Resume Craft.
  </Card>
</CardGroup>

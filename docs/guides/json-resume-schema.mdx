...
description: "Learn about the Resume Craft JSON Schema, available at /schema.json, and how it aims to become the gold standard for portable resume data."
---
...
Resume Craft aims to be the **gold standard in JSON Resume Schemas**. A universal, well-documented, and rigorously typed schema that any service can adopt to enable seamless resume portability across the entire job application ecosystem.

Imagine:

- Exporting your resume from Resume Craft as a JSON file
...
The Resume Craft JSON Schema is publicly available at:

<Info>
  [https://Resume-Craft.vercel.app/schema.json](https://Resume-Craft.vercel.app/schema.json)
</Info>

This endpoint returns a complete JSON Schema (draft 2020-12 compatible) that describes the structure of a Resume Craft document. It includes:
...
```json
{
  "$schema": "https://Resume-Craft.vercel.app/schema.json",
  "version": "5.0.0"
}
```

When importing resumes, Resume Craft uses this version to handle migrations and ensure backward compatibility.
...
const ajv = new Ajv();
const schema = await fetch("https://Resume-Craft.vercel.app/schema.json").then((r) => r.json());
...
- **Job portals** can accept Resume Craft JSON files directly
...
Below is the complete JSON Schema for Resume Craft. You can also fetch the latest version directly from [https://Resume-Craft.vercel.app/schema.json](https://Resume-Craft.vercel.app/schema.json).

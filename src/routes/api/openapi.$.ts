	...
	if (request.method === "GET" && request.url.endsWith("/spec.json")) {
		const spec = await openAPIGenerator.generate(router, {
			info: {
				...
				description: "Resume Craft API",
				license: { name: "MIT", url: "https://github.com/amruthpillai/Resume-Craft/blob/main/LICENSE" },
				contact: { name: "Amruth Pillai", email: "hello@amruthpillai.com", url: "https://amruthpillai.com" },
			},
			servers: [{ url: `${env.APP_URL}/api/openapi` }],
			externalDocs: { url: "https://docs.Resume-Craft.vercel.app", description: "Resume Craft Documentation" },
			commonSchemas: {
